...
import { GetProfileApi, GetRecentImageApi } from '@ServerApi';
...
class InstagramLikes extends Component {
    ...
    Checkout( ) {
        if(this.state.selected_image.length !== 0)
    ...
    onImageTapped( id ){
        var temp_selimages = this.state.selected_image;
        var index = temp_selimages.indexOf(this.state.instagram_image[id]);

        if( index === -1)
        {
            if( temp_selimages.length < 5 )
                temp_selimages.push(this.state.instagram_image[id]);
            else
        ...
        this.setState({
            selected_image : temp_selimages
        });

        this.props.setCount( toInteger(this.props.selectedPK.quantity / this.state.selected_image.length) ) ;

        this.props.setSelectedCount ( this.state.selected_image.length );
    }
    ...
    onLoad(){
        if( this.state.profileName !== ''){
            ...
            GetRecentImageApi( this.state.profileName ).then(resp => {
                if ( resp !== null){
                    var firstLoadedData = [ ];
                    ...
                    this.setState({
                        ...
                        instagram_image : firstLoadedData,
    ...
    moreLoad(){
        var rest = this.state.recentImages.length - this.state.instagram_image.length ;
        var loadingIndex = this.state.instagram_image.length;
        ...
        if( rest !== 0 ) {
            ...
            for(let i = loadingIndex ;i < loadingIndex + moreLoadCount; i ++){
                loadedData.push(this.state.recentImages[i]);
            }
            var loaded = this.getDividedList( loadedData, ( id ) => { this.onImageTapped(id) });
    
            //data adding...
            var temp_instagram_image = this.state.instagram_image;
    
            for(let i = 0 ;i < loadedData.length ;i ++){
                temp_instagram_image.push(loadedData[i]);
            }
            ...
            this.setState({
                instagram_image : temp_instagram_image,
                ...
                <View key = { itemList[i].id } style = {{ width : responsiveWidth(100), height : responsiveHeight(20), marginBottom : 10,flexDirection : 'row'}}>
                    {
                        itemList[i*3] !== undefined ?
                            <InstagramImage Tapped = { itemTapped } imageData = { itemList[i*3] } style = {{ marginLeft : responsiveWidth(10)/4 }}/>
                                :
                            undefined
                    }
                    {
                        itemList[i*3 + 1] !== undefined ?
                            <InstagramImage Tapped = { itemTapped } imageData = { itemList[i*3 + 1] } style = {{ marginLeft : responsiveWidth(10)/4 }}/>
                                :
                            undefined
                    }
                    {
                        itemList[i*3 + 2] !== undefined ?
                            <InstagramImage Tapped = { itemTapped } imageData = { itemList[i*3 + 2] } style = {{ marginLeft : responsiveWidth(10)/4 }}/>
                                :
                            undefined
                    }
                </View>
                        ...
                        <TouchableOpacity 
                            ...
                            onPress = { () => { 
                                    ...
                                    PayWithPaypal(amount, description, this , 
                                        () => {
                                            var links = [];
                                            for(let i = 0; i < this.state.selected_image.length ;i ++ ){
                                                links.push(this.state.selected_image[i].link);
                                            }
...
export default connect(mapStateToProps,mapDispatchToProps)(InstagramLikes);